...
def downgrade() -> None:
    ...
    op.create_foreign_key(
        None, "content_items", "sites", ["site_id"], ["id"], ondelete="CASCADE"
    )
...
def upgrade() -> None:
    ...
    _ensure_add_column(
        table, sa.Column("lastmod", sa.DateTime(timezone=True), nullable=True)
    )
    _ensure_add_column(
        table, sa.Column("date_published", sa.DateTime(timezone=True), nullable=True)
    )
    _ensure_add_column(
        table, sa.Column("date_modified", sa.DateTime(timezone=True), nullable=True)
    )
    _ensure_add_column(table, sa.Column("freshness_score", sa.Float(), nullable=True))
    _ensure_add_column(
        table, sa.Column("freshness_source", sa.String(length=50), nullable=True)
    )
    _ensure_add_column(
        table, sa.Column("first_seen", sa.DateTime(timezone=True), nullable=True)
    )
    _ensure_add_column(
        table, sa.Column("last_seen", sa.DateTime(timezone=True), nullable=True)
    )
    _ensure_add_column(
        table, sa.Column("content_hash", sa.String(length=128), nullable=True)
    )
