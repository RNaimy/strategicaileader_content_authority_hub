...
def upgrade() -> None:
    ...
    if _is_sqlite():
        ...
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_analytics_snapshots_id ON analytics_snapshots (id)"
        )
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_content_items_id ON content_items (id)"
        )
        ...
        op.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS ix_sites_domain ON sites (domain)"
        )
        return
    ...
    op.drop_constraint(
        op.f("content_items_site_id_fkey"), "content_items", type_="foreignkey"
    )
...
def downgrade() -> None:
    ...
    op.create_foreign_key(
        op.f("content_items_site_id_fkey"),
        "content_items",
        "sites",
        ["site_id"],
        ["id"],
