...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('captured_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.func.now()),
        ...
        sa.UniqueConstraint('site_id', 'captured_at', 'source', name='uq_snapshot_site_capture_source')
    )
...
def downgrade() -> None:
    ...
    op.drop_index('ix_analytics_snapshots_captured_at', table_name='analytics_snapshots')
