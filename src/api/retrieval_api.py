...
def semantic_search(
    ...
    domain: Optional[str] = Query(
        None, description="Restrict to a specific site by domain (e.g., example.com)"
    ),
    ...
    if site_id is not None and domain is not None:
        ...
        site_by_domain = (
            db.query(Site).filter(func.lower(Site.domain) == domain.lower()).first()
        )
        ...
        if site_by_id.id != site_by_domain.id:
            raise HTTPException(
                status_code=400, detail="site_id and domain refer to different sites"
            )
    ...
    candidates: List[ContentItem] = (
        query.offset(offset).limit(min(top_k * 5, 500)).all()
    )
    ...
    for c in candidates:
        ...
        if score > 0:
            results.append(
                RetrievedNode(id=c.id, url=c.url, title=c.title, score=score)
            )
...
class ReindexRequest(BaseModel):
    ...
    domain: Optional[str] = Field(
        None, description="Restrict reindex to a site by domain"
    )
...
def reindex_corpus(payload: ReindexRequest, db: Session = Depends(get_db)):
    ...
    if payload.site_id is not None and payload.domain is not None:
        ...
        by_domain = (
            db.query(Site)
            .filter(func.lower(Site.domain) == payload.domain.lower())
            .first()
        )
        if not by_domain:
            raise HTTPException(status_code=404, detail="Site (by domain) not found")
