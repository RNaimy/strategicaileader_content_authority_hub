...
from src.api.improvement_api import router as improvement_router
from src.api import clustering_router, content_router  # type: ignore

try:
    from src.api.sod_api import router as sod_router  # type: ignore
except Exception:  # pragma: no cover
    sod_router = None  # type: ignore[assignment]

try:
    from src.api.citation_api import router as citation_router  # type: ignore
except Exception:  # pragma: no cover
    citation_router = None  # type: ignore[assignment]
...
def root() -> dict:
    ...
    if "sod_router" in globals() and sod_router is not None:
        routers.append("/sod")
    if "citation_router" in globals() and citation_router is not None:
        routers.append("/citations")
